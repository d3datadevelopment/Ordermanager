...
return [
    ...
    'D3_ORDERMANAGER_SET_DEBUG_DESC'                     => "Activated test mode don't process the job. Execution shows job details on screen. Call the cronjob for check via the command line command. Verify the informations.",
    ...
    'D3_GENERAL_ORDERMANAGER_TASKCOUNT_CRONID'           => 'number of active tasks for Cronjob ID %1$s: %2$u',
    ...
    'D3_ORDERMANAGER_MAIN_FIELDSET_AFTERORDERCOMPLETED'  => 'after completion of the order by the customer (included from the standard licence)',
    'D3_ORDERMANAGER_MAIN_FIELDSET_AFTERORDERSAVE'       => 'after each saving of the order (included from the standard licence)',
    'D3_ORDERMANAGER_MAIN_FIELDSET_EVENTTRIGGER'         => 'programmable trigger (included from the premium licence)',
    ...
    'D3_ORDERMANAGER_MAIN_EVENTID'                       => 'event name',
    'D3_ORDERMANAGER_MAIN_EVENTID_DESC'                  => 'The name of the event must be set for programmable triggers. This name is used to find all assigned tasks. If several tasks are given the same name, they are executed one by one at the start of the event.',
    'D3_ORDERMANAGER_MAIN_PREEXECUTESCRIPT'              => 'pre-executed script',
    'D3_ORDERMANAGER_MAIN_PREEXECUTESCRIPT_DESC'         => 'Directly before the task execution via cronjob a script can be executed which can perform preparatory work. This specification is optional. The specification of a statically callable function including its complete class name is expected. e.g.: \D3\Example\prePostExecute::preExecuteScript  The first parameter passed is the task object (d3ordermanager). The script execution is only possible in the Premium Edition of the module. If necessary, extend your license for this.',
    'D3_ORDERMANAGER_MAIN_POSTEXECUTESCRIPT'             => 'post-executed script',
    'D3_ORDERMANAGER_MAIN_POSTEXECUTESCRIPT_DESC'        => 'Directly after the task execution via cronjob a script can be executed, which can do follow-up work. This specification is optional. The specification of a statically callable function including its complete class name is expected. e.g.: \D3\Example\prePostExecute::postExecuteScript  The first parameter passed is the task object (d3ordermanager). The script execution is only possible in the Premium Edition of the module. If necessary, extend your license for this.',
    ...
    'D3_ORDERMANAGER_MAIN_EXECEVENTTRIGGERED'            => 'Execution possible by any programmed trigger of the event',
    'D3_ORDERMANAGER_MAIN_EXECEVENTTRIGGERED_DESC'       => 'If this box is ticked, this task is executed when the ‘CustomEvent’ with the event name set here is triggered via the Symfony Event Dispatcher. For details, please refer to the manual at https://docs.oxidmodule.com/.',
    ...
    'D3_ORDERMANAGER_REQU_MAILREGEXP_DESC'               => 'Insert the search pattern for the mail address filter. You can use the following wildcards: [ _ ] a single sign, empty is possible [ % ] one or many signs. If you wanna use the wildcard signs as search string, mask it with a slash [ / ]. Examples: "%@gmx.example" find all addresses from the domain gmx.example "abc@rtl_.example", find the addresses from rtl.example and from rtl2.example too. The case sensitivity has no matter.',
    ...
    'D3_ORDERMANAGER_REQU_ORDEREXECUTEMETHOD_DESC'       => 'Only specify the name of the method here. Parameters or parentheses must not be used. The method must exist "public" on the order list object ("OrderList / ListModel" and its extensions). The method call is not static. Within the method, the oxOrderList / oxList object can be arbitrarily manipulated. It is not necessary to return the job list. These parameters are passed to the method: * currently used job manager job object ("d3ordermanager" or its overload). A return value is not processed. Using a programmed filter will cause an increased execution load depending on the result set. In principle, the other filters should be used in preference. Even when using this option, try to pre-select the hit list with one of the other filters.',
    ...
    'D3_ORDERMANAGER_ACTION_ORDEREXECUTEMETHOD_DESC'     => 'Only specify the name of the method here. Parameters or parentheses must not be used. The method must exist "public" on the order object ("Order" and its extensions). The method call is not static. These parameters are passed to the method: * Currently used job manager job object ("d3ordermanager" or its overload). The expected return value is a boolean that defines whether the order object needs to be saved.',
    ...
    'D3_ORDERMANAGER_ACTION_ORDERSENDPUSHNOTIFICATION_URL_DESC' => 'Enter the URL of the service or script that is to receive the message. If the URL should contain dynamic parameters, put them in double curly brackets "{{value1}}". These parameters are available in the standard and are also given as a body: * value1 + orderNr -> order number * value2 + userName -> mail of the orderer * value3 + shopUrl -> URL of this shop. When using the POST method, optional GET parameters are appended to the body.',
    'D3_ORDERMANAGER_ACTION_ORDERSENDPUSHNOTIFICATION_METHOD' => 'transmission method',
    'D3_ORDERMANAGER_ACTION_ORDERSENDPUSHNOTIFICATION_DESC' => 'Select the preferred transmission method (POST is the default for cases).',
    ...
    'D3_ORDERMANAGER_ACTION_EXPORTTEMPLATE_DESC'         => 'Enter here the name of the template containing the structure of the export (ex .: "@d3ordermanager/admin/export-templates/CSV-Vorlage"). Examples of export templates can be found in the module package under "vendor/d3/ordermanager/views/*/admin/export-templates". Create a template for your own formats.',
    ...
    'D3_ORDERMANAGER_CRON_STAT_TASK'                     => 'task',
    'D3_ORDERMANAGER_CRON_STAT_LASTEXECTIME'             => 'last execution time',
    'D3_ORDERMANAGER_CRON_STAT_NOT_YET_EXECUTED'         => 'not yet executed',
    'D3_ORDERMANAGER_CRON_STAT_ORDERSFROM'               => 'orders from %1$s',
    'D3_ORDERMANAGER_CRON_STAT_CHANGES'                  => 'changes',
    'D3_ORDERMANAGER_CRON_STAT_CHANGES_NEW'              => 'new',
    'D3_ORDERMANAGER_CRON_STAT_ID'                       => 'internal ID',

    'SHOP_MODULE_GROUP_d3_ordermanager_general'          => 'general settings',
    'SHOP_MODULE_d3_ordermanager_setting_keptlogfiles'   => 'Number of log files to be stored',
    'HELP_SHOP_MODULE_d3_ordermanager_setting_keptlogfiles'    => 'A new file is created for each day with a log occurrence. Older files are automatically deleted.',
    'SHOP_MODULE_d3_ordermanager_setting_logErrorsOnly'  => 'Log entries are only written in the case of an error',
];
