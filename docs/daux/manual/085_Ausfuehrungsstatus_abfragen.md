---
title: Ausführungsstatus abfragen
---
...
```
./vendor/bin/d3_ordermanager_cron --no-colors statistic [shop-id]
```

### Anzahl aktiver Aufgaben und letztes Ausführungsdatum zu einer bestimmten Cronjob-ID

```
./vendor/bin/d3_ordermanager_cron --no-colors status [shop-id] [cronjob-id]
```
