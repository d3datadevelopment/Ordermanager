        ...
        <dd>
            [{if $oView->isEditMode()}]
                [{block name="ordermanager_admin_requ_orderfield_editor"}]
                    ...
                    <input id="RequOrderFieldTypeNoContent" type="radio" style="margin-left: 30px;" name="value[sCheckOrderFieldType]" value="nocontent" [{if $edit->getValue('sCheckOrderFieldType') == 'nocontent'}]checked[{/if}] [{$readonly}] [{$blActionRestriction}]>
                    <label for="RequOrderFieldTypeNoContent">[{oxmultilang ident="D3_ORDERMANAGER_REQU_ORDERFIELDVALUETYPE_NOCONTENT"}]</label>
                    <label for="sOrderFieldNoValue" style="position: absolute; left: -2000px">[{oxmultilang ident="D3_ORDERMANAGER_REQU_ORDERFIELDNOVALUE"}]</label>
                    <input id="sOrderFieldNoValue" class="editinput" type="text" size="30" maxlength="100" name="value[sOrderField_FieldNoValue]" value="[{$edit->getValue('sOrderField_FieldNoValue')}]" [{$readonly}] [{$blActionRestriction}]>
                    [{oxinputhelp ident="D3_ORDERMANAGER_REQU_ARTICLECONTENT2_DESC"}]<br>
            ...
            [{else}]
                [{block name="ordermanager_admin_requ_orderfield_viewer"}]
                    ...
                    [{if $edit->getValue('sCheckOrderFieldType') == 'content' || $edit->getValue('sCheckOrderFieldType') == ''}]
                        ...
                        [{oxinputhelp ident="D3_ORDERMANAGER_REQU_ARTICLECONTENT2_DESC"}]<br>
                    [{/if}]
                    [{if $edit->getValue('sCheckOrderFieldType') == 'nocontent'}]
                        [{oxmultilang ident="D3_ORDERMANAGER_REQU_ORDERFIELDVALUETYPE_NOCONTENT"}] [{$edit->getValue('sOrderField_FieldNoValue')}]
                        [{oxinputhelp ident="D3_ORDERMANAGER_REQU_ARTICLECONTENT2_DESC"}]<br>
                    [{/if}]
